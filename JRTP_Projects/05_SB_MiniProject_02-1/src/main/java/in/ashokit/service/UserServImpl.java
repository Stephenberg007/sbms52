...
public class UserServImpl implements UserServ {
	...
	public boolean registerUser(UserRegistrationDTO userRegDto) {
	 ...
	 CountryEntity country = counRepo.findById(userRegDto.getCountryId()).get();// findBy() produces Optional object so we use get()
	...
	public Map<Integer, String> getCountries() {
		...
		return counMap;
		/*If you return a List<CountryEntity>, the caller of getCountries() would have to:
					1) Iterate over the list manually.
					2) Extract countryId and countryName themselves
					
When Should You Use a List vs. a Map?
						✅ Use a List<CountryEntity> when:

The caller wants access to the full CountryEntity object.
You need to preserve insertion order.
You want to keep the method flexible for various use cases.
✅ Use a Map<Integer, String> when:

You only need country ID and name (not the full object).
You want fast lookups by country ID.
